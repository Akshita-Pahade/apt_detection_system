"""
SecureBank APT Detection Engine — Streamlit Dashboard + Flask API
=================================================================
Run dashboard:  streamlit run app.py
Run API only:   python server.py
"""
...
# ── Flask in-memory state ─────────────────────────────────────────────────────
flask_app = Flask(__name__)

locked_users = {}
unlock_log = []

live_event_store = defaultdict(list)
live_event_store_date = datetime.now().strftime("%Y-%m-%d")
live_analysis_results = {}

EVENT_SERVER_PORT = int(os.environ.get("SECUREBANK_PORT", "5000"))
EVENT_SERVER_URL = f"http://localhost:{EVENT_SERVER_PORT}"

_flask_engine = None
_flask_engine_lock = threading.Lock()


def get_flask_engine():
    global _flask_engine
    with _flask_engine_lock:
        if _flask_engine is None:
            import apt_engine
            _flask_engine = apt_engine.APTEngine()
            _flask_engine.loadConfig("config.txt")
        return _flask_engine


# ── Shared parsing / building (used by Flask + Streamlit) ─────────────────────
...
def parse_path(output: str):
    ...
    m2 = re.search(r"Visited path:\s*(.+)", output)
    if m2:
...
def parse_score_breakdown(output: str) -> list:
    lines = re.findall(r"^\s+(.+?)\s+\+(\d+)\s*$", output, re.MULTILINE)
    return [
        {"reason": r.strip(), "points": int(s)}
        for r, s in lines
        if r.strip() not in ("TOTAL RISK SCORE", "Flag / Reason")
    ]


def build_session_options(meta: dict) -> dict:
    opts = {}
    if meta.get("last_active_date"):
        opts["last_active_date"] = str(meta["last_active_date"])
    if meta.get("resignation_date"):
        opts["resignation_date"] = str(meta["resignation_date"])
    if meta.get("normal_device_count") is not None:
        opts["normal_device_count"] = int(meta["normal_device_count"])
    if meta.get("failed_attempts"):
        opts["failed_attempts"] = int(meta["failed_attempts"])
    if meta.get("last_failed_ip"):
        opts["last_failed_ip"] = meta["last_failed_ip"]
    if meta.get("source_ip"):
        opts["source_ip"] = meta["source_ip"]
    return opts


def build_event_dict(ev: dict, session_meta: dict, is_first: bool = False) -> dict:
    d = {
        "device": ev.get("device", ""),
        "date": ev.get("date", str(date.today())),
        "hour": int(ev.get("hour", ev.get("event_hour", 9))),
        "minute": int(ev.get("minute", ev.get("event_minute", 0))),
        "location": ev.get("location", "Mumbai"),
        "recordsAccessed": int(ev.get("records_accessed", ev.get("recordsAccessed", 0))),
    }
    if is_first and session_meta.get("failed_attempts"):
        d["failedAttempts"] = int(session_meta["failed_attempts"])
        d["lastFailedIP"] = session_meta.get("last_failed_ip", "")
        d["sourceIP"] = session_meta.get("source_ip", "")
    elif is_first and session_meta.get("source_ip"):
        d["sourceIP"] = session_meta.get("source_ip", "")
    return d


def live_verdict_label(score: int) -> str:
    if score >= 30:
        return "CRITICAL"
    if score >= 20:
        return "HIGH"
...
def _alert_level(score: int, reasons: list) -> str:
    _CRITICAL = {"phantom edge", "apt signature", "unknown device", "apt path", "unauthorized apt"}
    reason_text = " ".join(r.lower() for r in reasons)
    has_critical = any(k in reason_text for k in _CRITICAL)
...
def _build_email(user: str, score: int, threshold: int, alert_level: str,
                 score_lines: list, authorized_but_anomalous: bool,
                 source: str = "manual") -> tuple:
    if source == "live":
        vlabel = live_verdict_label(score)
        subject = f"[LIVE ALERT] SecureBank APT Detection — {user} | Score: {score} | {vlabel}"
    else:
    ...
    src_note = "Live client event stream" if source == "live" else "Manual session analysis"
    body = (
...
def _send_email_alert(user: str, score: int, threshold: int, alert_level: str,
                      score_lines: list, authorized_but_anomalous: bool = False,
                      source: str = "manual") -> bool:
    ps_path = os.path.join(os.path.dirname(__file__), "send_alert.ps1")
    if not os.path.exists(ps_path):
        return False
    subject, body = _build_email(
        user, score, threshold, alert_level, score_lines,
        authorized_but_anomalous, source=source,
    )
    try:
...
def _maybe_reset_live_store():
    global live_event_store_date
    today = datetime.now().strftime("%Y-%m-%d")
    if today != live_event_store_date:
        live_event_store.clear()
        live_event_store_date = today


def _store_incoming_event(incoming: dict) -> dict:
    return {
        "device": incoming.get("device", ""),
        "date": str(incoming.get("event_date", incoming.get("date", date.today()))),
        "hour": int(incoming.get("event_hour", incoming.get("hour", 0))),
        "minute": int(incoming.get("event_minute", incoming.get("minute", 0))),
        "location": incoming.get("location", "Mumbai"),
        "records_accessed": int(incoming.get("records_accessed", incoming.get("recordsAccessed", 0))),
        "source_ip": incoming.get("source_ip", incoming.get("client_ip", "")),
        "failed_attempts": int(incoming.get("failed_attempts", 0)),
        "last_failed_ip": incoming.get("last_failed_ip", ""),
        "machine_name": incoming.get("machine_name", ""),
        "received_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }


def _run_live_analysis(username: str, stored_events: list) -> dict:
    engine = get_flask_engine()
    session_meta = {
        "failed_attempts": stored_events[0].get("failed_attempts", 0) if stored_events else 0,
        "last_failed_ip": stored_events[0].get("last_failed_ip", "") if stored_events else "",
        "source_ip": stored_events[0].get("source_ip", "") if stored_events else "",
    }
    ev_list = [
        build_event_dict(ev, session_meta, is_first=(i == 0))
        for i, ev in enumerate(stored_events)
    ]
    opts = build_session_options(session_meta)
    try:
        output = engine.analyzeSession(username, ev_list, opts)
    except TypeError:
        output = engine.analyzeSession(username, ev_list)

    score = parse_score(output)
    breakdown = parse_score_breakdown(output)
    visited = parse_path(output)
    verdict = live_verdict_label(score)
    top_reason = breakdown[0]["reason"] if breakdown else "High risk score"
    return {
        "score": score,
        "verdict": verdict,
...
def ping():
    return jsonify({"status": "ok", "service": "SecureBank Event Server"})


@flask_app.route("/locked_users", methods=["GET"])
def get_locked_users():
    return jsonify(locked_users)


@flask_app.route("/status/<username>", methods=["GET"])
def get_status(username):
    user = _normalize_username(username)
    if user in locked_users:
        return jsonify({"user": user, "status": "locked", "details": locked_users[user]})
    return jsonify({"user": user, "status": "active"})


@flask_app.route("/unlock/<username>", methods=["POST"])
def unlock_user(username):
    ...
    if user in locked_users:
        ...
        return jsonify({"status": "unlocked", "user": user})
    return jsonify({"status": "not_locked", "user": user})


@flask_app.route("/live_results", methods=["GET"])
def get_live_results():
    return jsonify(live_analysis_results)


@flask_app.route("/live_events/<username>", methods=["GET"])
def get_live_events(username):
    user = _normalize_username(username)
    return jsonify(live_event_store.get(user, []))


@flask_app.route("/reset_live/<username>", methods=["POST"])
def reset_live(username):
    user = _normalize_username(username)
    live_event_store[user] = []
    if user in live_analysis_results:
        del live_analysis_results[user]
    return jsonify({"status": "reset", "user": user})


@flask_app.route("/events", methods=["GET"])
def get_events():
    """Return accumulated in-memory events (all users), most recent first."""
    all_events = []
    for user, evs in live_event_store.items():
        for ev in evs:
            row = dict(ev)
            row["username"] = user
            all_events.append(row)
    all_events.sort(key=lambda x: x.get("received_at", ""), reverse=True)
...
def post_events():
    ...
    events_in = data if isinstance(data, list) else [data]
    client_ip = request.remote_addr or ""
    required = ("machine_name", "username", "device", "event_date", "event_hour", "event_minute")

    results = []
    for incoming in events_in:
        missing = [f for f in required if f not in incoming]
        if missing:
            return jsonify({"error": f"Missing fields: {', '.join(missing)}"}), 400

        username = _normalize_username(incoming["username"])
        if username in locked_users:
            return jsonify({"status": "blocked", "reason": "account_locked"}), 403

        _maybe_reset_live_store()
        stored = _store_incoming_event({**incoming, "client_ip": incoming.get("client_ip", client_ip)})
        live_event_store[username].append(stored)

        analysis = _run_live_analysis(username, live_event_store[username])
        score = analysis["score"]
        verdict = analysis["verdict"]
        breakdown = analysis["score_breakdown"]
        top_reason = analysis["top_reason"]

        prev = live_analysis_results.get(username, {})
        email_sent = prev.get("email_sent", False)

        live_analysis_results[username] = {
            "score": score,
            "verdict": verdict,
            "score_breakdown": breakdown,
            "visited_path": analysis["visited_path"],
            "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "event_count": len(live_event_store[username]),
        ...
        threshold = 30
        try:
            threshold = get_flask_engine().getRiskThreshold()
        except Exception:
            pass

        score_lines = [(b["reason"], b["points"]) for b in breakdown]
        reason_labels = [r for r, _ in score_lines]
        al = _alert_level(score, reason_labels)

        if score >= 30 and not email_sent:
            success = _send_email_alert(
                username, score, threshold, al, score_lines,
                authorized_but_anomalous=False, source="live",
            )
            if success:
                live_analysis_results[username]["email_sent"] = True

        if score >= 75 and username not in locked_users:
            ...
            live_analysis_results[username]["locked"] = True

        results.append({"status": "ok", "score": score, "verdict": verdict, "user": username})
...
def _start_flask_daemon():
    if os.environ.get("SECUREBANK_FLASK_STARTED") == "1":
        return
    os.environ["SECUREBANK_FLASK_STARTED"] = "1"

    def _run():
        flask_app.run(host="0.0.0.0", port=EVENT_SERVER_PORT, debug=False, use_reloader=False)

    t = threading.Thread(target=_run, daemon=True, name="SecureBankFlask")
    t.start()


if os.environ.get("SECUREBANK_SERVER_ONLY") != "1":
    _start_flask_daemon()


def api_get(path: str):
    resp = requests.get(f"{EVENT_SERVER_URL}{path}", timeout=5)
    resp.raise_for_status()
    return resp.json()


def api_post(path: str):
    resp = requests.post(f"{EVENT_SERVER_URL}{path}", timeout=5)
    resp.raise_for_status()
    return resp.json()


def fetch_locked_users_api() -> dict:
    try:
        return api_get("/locked_users")
    except Exception:
        return dict(locked_users)


def live_verdict_color(verdict: str) -> str:
    v = (verdict or "").upper()
    if v == "CRITICAL":
        return "#ff2d6b"
    if v == "HIGH":
...
# ── Streamlit dashboard (skipped when SECUREBANK_SERVER_ONLY=1) ───────────────
if os.environ.get("SECUREBANK_SERVER_ONLY") != "1":
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    # HELPERS (dashboard-specific; shared parse/build/email at module level)
    # ─────────────────────────────────────────────────────────────────────────────
    ...
    def render_events_table(rows: list, columns: list) -> str:
        """Render a dark-themed HTML table (avoids st.dataframe Glide grid visibility issues)."""
        th_style = (
            "padding:10px 14px;text-align:left;font-family:'Share Tech Mono',monospace;"
            "font-size:.72rem;letter-spacing:.08em;color:#7986a8;"
            "background:#1e2335;border-bottom:1px solid #2a3352;"
        )
        td_style = (
            "padding:9px 14px;font-family:'Share Tech Mono',monospace;"
            "font-size:.78rem;color:#cdd6f4;border-bottom:1px solid #2a3352;"
        )
        thead = "".join(f"<th style='{th_style}'>{col}</th>" for col in columns)
        tbody_rows = []
        for i, row in enumerate(rows):
            bg = "#181c27" if i % 2 == 0 else "#141824"
            cells = "".join(
                f"<td style='{td_style}background:{bg};'>{row.get(col, '—')}</td>"
                for col in columns
            )
            tbody_rows.append(f"<tr>{cells}</tr>")
        return (
            f'<div style="overflow-x:auto;border:1px solid #2a3352;border-radius:6px;margin-top:8px">'
            f'<table style="width:100%;border-collapse:collapse;background:#181c27">'
            f"<thead><tr>{thead}</tr></thead>"
            f"<tbody>{''.join(tbody_rows)}</tbody>"
            f"</table></div>"
        )

    # ── Extended threat detection (Cases 1–8) ─────────────────────────────────────
    INDIAN_CITIES = [
        "Mumbai", "Delhi", "Bangalore", "Chennai", "Kolkata",
        "Hyderabad", "Pune", "Ahmedabad", "Jaipur", "Lucknow",
    ]

    EXTENDED_THREAT_CASES = [
        ("Geographic Jump",       "geographic jump",          50),
        ("Simultaneous Sessions", "simultaneous sessions",    45),
        ("Dormant Account",       "dormant account",          35),
        ("Post-Resignation",      "post-resignation",         60),
        ("Brute Force Pattern",   "brute force",              55),
        ("Data Volume Spike",     "data volume spike",        50),
        ("Behavioral Drift",      "behavioral drift",         40),
        ("Reverse Path Access",   "reverse path access",      45),
    ]

    def match_extended_threats(score_items: list) -> list:
        """Return list of (name, triggered, points, reason) for each extended case."""
        results = []
        for name, keyword, default_pts in EXTENDED_THREAT_CASES:
            triggered = False
            pts = 0
            reason = ""
            for si in score_items:
                r = si.get("reason", "") if isinstance(si, dict) else si[0]
                p = si.get("points", 0) if isinstance(si, dict) else si[1]
                if keyword in r.lower():
                    triggered = True
                    pts += p
                    if not reason:
                        reason = r
            results.append((name, triggered, pts if triggered else 0, reason))
        return results

    def render_extended_threat_badges(matched: list) -> str:
        badges = []
        for name, triggered, pts, _ in matched:
            if triggered:
                badges.append(
                    f'<span style="display:inline-block;margin:4px 6px 4px 0;padding:4px 10px;'
                    f'background:#2b0010;border:1px solid #ff2d6b;border-radius:4px;'
                    f'font-family:\'Share Tech Mono\',monospace;font-size:.72rem;color:#ff2d6b">'
                    f'⚠ {name} (+{pts})</span>'
                )
            else:
                badges.append(
                    f'<span style="display:inline-block;margin:4px 6px 4px 0;padding:4px 10px;'
                    f'background:#001a06;border:1px solid #39ff14;border-radius:4px;'
                    f'font-family:\'Share Tech Mono\',monospace;font-size:.72rem;color:#39ff14">'
                    f'✓ {name}</span>'
                )
        return "".join(badges)

    def get_demo_sessions():
        """Eight hardcoded demo sessions — one per extended threat case."""
        today = date.today()
        yesterday = str(today - timedelta(days=1))
        last_active_120 = str(today - timedelta(days=120))
        session_date = str(today)

        return [
            {
                "label": "Case 1 — Geographic Jump (PRIYA)",
                "user": "PRIYA",
                "events": [
                    {"device": "WS-TELLER-01", "date": session_date, "hour": 9, "minute": 0,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "WS-TELLER-01", "date": session_date, "hour": 10, "minute": 30,
                     "location": "Delhi", "records_accessed": 0},
                ],
                "meta": {},
            },
            {
                "label": "Case 2 — Simultaneous Sessions (RAJAN)",
                "user": "RAJAN",
                "events": [
                    {"device": "WS-OPS-02", "date": session_date, "hour": 14, "minute": 30,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "INTRA-SRV-01", "date": session_date, "hour": 14, "minute": 30,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "REPORT-SRV-01", "date": session_date, "hour": 14, "minute": 30,
                     "location": "Mumbai", "records_accessed": 0},
                ],
                "meta": {},
            },
            {
                "label": "Case 3 — Dormant Account (SNEHA)",
                "user": "SNEHA",
                "events": [
                    {"device": "VAULT-DB-01", "date": session_date, "hour": 10, "minute": 0,
                     "location": "Mumbai", "records_accessed": 0},
                ],
                "meta": {"last_active_date": last_active_120},
            },
            {
                "label": "Case 4 — Post-Resignation (VIKRAM)",
                "user": "VIKRAM",
                "events": [
                    {"device": "CORE-TXN-SRV", "date": session_date, "hour": 11, "minute": 0,
                     "location": "Mumbai", "records_accessed": 0},
                ],
                "meta": {"resignation_date": yesterday},
            },
            {
                "label": "Case 5 — Brute Force (PRIYA)",
                "user": "PRIYA",
                "events": [
                    {"device": "WS-TELLER-01", "date": session_date, "hour": 8, "minute": 15,
                     "location": "Mumbai", "records_accessed": 0},
                ],
                "meta": {
                    "failed_attempts": 15,
                    "last_failed_ip": "192.168.1.10",
                    "source_ip": "10.0.0.5",
                },
            },
            {
                "label": "Case 6 — Data Volume Spike (RAJAN)",
                "user": "RAJAN",
                "events": [
                    {"device": "WS-OPS-02", "date": session_date, "hour": 15, "minute": 0,
                     "location": "Mumbai", "records_accessed": 8000},
                ],
                "meta": {},
            },
            {
                "label": "Case 7 — Behavioral Drift (SNEHA)",
                "user": "SNEHA",
                "events": [
                    {"device": "WS-LOAN-03", "date": session_date, "hour": 9, "minute": 0,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "INTRA-SRV-01", "date": session_date, "hour": 9, "minute": 10,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "REPORT-SRV-01", "date": session_date, "hour": 9, "minute": 20,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "WS-OPS-02", "date": session_date, "hour": 9, "minute": 30,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "WS-TELLER-01", "date": session_date, "hour": 9, "minute": 40,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "CORE-TXN-SRV", "date": session_date, "hour": 9, "minute": 50,
                     "location": "Mumbai", "records_accessed": 0},
                ],
                "meta": {"normal_device_count": 2},
            },
            {
                "label": "Case 8 — Reverse Path (VIKRAM)",
                "user": "VIKRAM",
                "events": [
                    {"device": "VAULT-DB-01", "date": session_date, "hour": 16, "minute": 0,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "CORE-TXN-SRV", "date": session_date, "hour": 16, "minute": 15,
                     "location": "Mumbai", "records_accessed": 0},
                    {"device": "WS-TELLER-01", "date": session_date, "hour": 16, "minute": 30,
                     "location": "Mumbai", "records_accessed": 0},
                ],
                "meta": {},
            },
    ...
    def run_demo_sessions(engine):
        """Analyse all 8 demo sessions on first run."""
        for demo in get_demo_sessions():
            ev_list = [
                build_event_dict(e, demo["meta"], is_first=(i == 0))
                for i, e in enumerate(demo["events"])
            ]
            opts = build_session_options(demo["meta"])
            try:
                engine.analyzeSession(demo["user"], ev_list, opts)
            except TypeError:
                engine.analyzeSession(demo["user"], ev_list)

    def load_demo_into_form(demo: dict):
        """Populate session state from a demo definition."""
        st.session_state.events = [dict(e) for e in demo["events"]]
        st.session_state.session_meta = dict(demo.get("meta", {}))
        st.session_state.selected_user = demo["user"]
    ...
    if "events" not in st.session_state:
        st.session_state.events = [{
            "date": str(date.today()), "hour": 9, "minute": 0,
            "device": "", "location": "Mumbai", "records_accessed": 0,
        }]
    if "session_meta" not in st.session_state:
        st.session_state.session_meta = {
            "last_active_date": None,
            "resignation_date": None,
            "normal_device_count": 2,
            "failed_attempts": 0,
            "last_failed_ip": "",
            "source_ip": "",
        }
    if "last_score_items" not in st.session_state:
        st.session_state.last_score_items = []
    ...
    # Run demo sessions once on first load
    if engine_obj and config_loaded and not st.session_state.demo_loaded:
        run_demo_sessions(engine_obj)
        st.session_state.demo_loaded = True
        demos = get_demo_sessions()
        load_demo_into_form(demos[0])
    ...
    def render_locked_accounts_panel(key_prefix: str = ""):
        """Show locked accounts table with unlock buttons."""
        st.markdown(sec_header("// LOCKED ACCOUNTS"), unsafe_allow_html=True)
        try:
            locked = fetch_locked_users_api()
        except Exception:
            locked = {}
        ...
        if not locked:
            st.markdown(
                ...
                'color:#39ff14;padding:8px 0">✓ No accounts currently locked</div>',
                unsafe_allow_html=True,
            )
            return

        for user, info in sorted(locked.items()):
            c1, c2, c3, c4, c5 = st.columns([2, 1, 3, 2, 1])
            with c1:
                st.markdown(f"**{user}**")
            with c2:
                st.markdown(f"`{info.get('score', '—')}`")
            with c3:
                st.markdown(info.get("reason", "—")[:60])
            with c4:
                st.markdown(info.get("locked_at", "—"))
            with c5:
                if st.button("🔓 Unlock", key=f"{key_prefix}unlock_{user}"):
                    try:
                        api_post(f"/unlock/{user}")
                        st.success(f"✓ {user} unlocked successfully")
                        st.rerun()
                    except Exception as ex:
                        st.error(f"Unlock failed: {ex}")
    ...
    def render_session_path_graph(path_nodes, score, output):
        ...
        for i in range(len(path_nodes) - 1):
            ...
            # Check if this specific hop is called out as a phantom in the output
            if re.search(
                rf'PHANTOM\s*EDGE[^\n]*{re.escape(src)}[^\n]*{re.escape(dst)}'
                rf'|{re.escape(src)}[^\n]*{re.escape(dst)}[^\n]*PHANTOM',
                output, re.IGNORECASE
            ):
                phantom_pairs.add((src, dst))
            # Also flag any hop that has no matching topology edge
            elif (src, dst) not in all_edges and (dst, src) not in all_edges:
                phantom_pairs.add((src, dst))
        ...
        tier_slots = {0: 0, 1: 0, 2: 0}
        for d in all_devices:
            nid  = d["id"]
            tier = zone_tier.get(nid, 0)
            slot = tier_slots[tier]
            tier_slots[tier] += 1
            count = tier_counts[tier]
            x = tier * 5.5
            # Evenly space nodes vertically and center them
            y = (slot - (count - 1) / 2.0) * 2.8
            positions[nid] = (x, y)
        ...
        def edge_style(f, t):
            """Returns (color, width, is_active, is_phantom)"""
            is_phantom = (f, t) in phantom_pairs or (t, f) in phantom_pairs
            if is_phantom:
                return ("#ff00ff", 3, True, True)   # magenta dashed = phantom hop
            if (f, t) in visited_edges or (t, f) in visited_edges:
                if is_threat:     return ("#ff2d6b", 5, True, False)
                if is_suspicious: return ("#ffa726", 4, True, False)
                return ("#4fc3f7", 4, True, False)
            return ("#4a6494", 2.0, False, False)

        # ── Build edge traces ──
        edge_traces = []

        # Draw phantom edges first (hops with no topology edge)
        for i in range(len(path_nodes) - 1):
            src, dst = path_nodes[i], path_nodes[i+1]
            if (src, dst) in phantom_pairs and src in positions and dst in positions:
                x0, y0 = positions[src]
                x1, y1 = positions[dst]
                # Glow
                edge_traces.append(go.Scatter(
                    ...
                    mode="lines",
                    line=dict(color="rgba(255,0,255,0.18)", width=18),
                ...
                ax = x0 + (x1 - x0) * 0.75
                ay = y0 + (y1 - y0) * 0.75
                angle = _math.degrees(_math.atan2(y1 - y0, x1 - x0))
                edge_traces.append(go.Scatter(
                    x=[ax], y=[ay], mode="markers",
                    marker=dict(symbol="arrow", size=13, color="#ff00ff", angle=angle,
                                line=dict(width=0)),
                    hoverinfo="none", showlegend=False,
                ))
        ...
        for f, t in all_edges:
            ...
            x0, y0 = positions[f]
            x1, y1 = positions[t]
            ec, ew, is_active, is_phantom = edge_style(f, t)
            if is_phantom:
        ...
        pad_x = max(1.5, (max(all_x) - min(all_x)) * 0.14)
        pad_y = max(1.2, (max(all_y) - min(all_y)) * 0.18)
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                showgrid=False, zeroline=False, showticklabels=False,
                range=[min(all_y) - pad_y - 0.8, max(all_y) + pad_y + 0.5],
                scaleanchor=None,
                constrain="domain",
            ),
            hovermode="closest",
        ...
        ] + ([("#ff00ff", "Phantom Edge (no topology link)")] if phantom_pairs else [])
        # deduplicate
        ...
        legend_html = "".join(
            ...
            f'<svg width="12" height="12" style="flex-shrink:0"><circle cx="6" cy="6" r="6" '
            f'fill="{c}" filter="url(#g)"/></svg>'
            f'<span style="font-family:\'Share Tech Mono\',monospace;font-size:.68rem;'
            f'color:#7986a8 !important">{l}</span></span>'
    ...
    def page_session_analysis():
        ...
        with col_left:
            ...
            user_index = usernames.index(default_user) if default_user in usernames else 0
            ...
            locked_map    = fetch_locked_users_api()
            user_locked   = bool(sel_user and sel_user in locked_map)
            status_cfg = {
            ...
            if user_locked:
                s_color, s_icon, s_label = "#ff2d6b", "🔒", "LOCKED"
            else:
                s_color, s_icon, s_label = status_cfg.get(alert_status, ("#5a7099", "●", alert_status.upper()))
            lock_badge = ""
            if user_locked:
                lock_badge = (
                    '<br><span style="color:#ff2d6b;font-size:.85rem;letter-spacing:.08em">'
                    '🔒 ACCOUNT LOCKED</span>'
                )
            st.markdown(
                f'<div style="background:#0d1428;border:1px solid #1e2d50;border-radius:6px;'
                ...
                f'<span style="color:{score_col};font-size:1.3rem;font-weight:700">{live_score}</span>'
                f'{lock_badge}<br>'
            ...
            render_locked_accounts_panel(key_prefix="sa_")
            ...
            if st.session_state.history:
                ...
                for h in reversed(st.session_state.history[-5:]):
                    sc  = h["score"]
                    col = score_color(sc)
                    lock_icon = " 🔒" if h.get("locked") or sc >= 75 else ""
                    st.markdown(
                        ...
                        f'<span style="color:{col}">{sc}{lock_icon}</span>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )

        with col_right:
            st.markdown(sec_header("// MANUAL SESSION FORM"), unsafe_allow_html=True)

            # Demo session loader
            demos = get_demo_sessions()
            demo_labels = ["— Select demo to load —"] + [d["label"] for d in demos]
            ...
            if sel_demo != "— Select demo to load —":
                demo_idx = demo_labels.index(sel_demo) - 1
                if st.button("Load Selected Demo", key="load_demo_btn"):
                    load_demo_into_form(demos[demo_idx])
                    st.rerun()

            meta = st.session_state.session_meta

            with st.expander("Login Attempt Details", expanded=False):
                meta["failed_attempts"] = st.number_input(
                    "Failed Attempts", min_value=0, value=int(meta.get("failed_attempts", 0)),
                    key="meta_failed_attempts",
                )
                meta["last_failed_ip"] = st.text_input(
                    "Last Failed IP", value=meta.get("last_failed_ip", ""),
                    placeholder="192.168.1.10", key="meta_last_failed_ip",
                )
                meta["source_ip"] = st.text_input(
                    "Source IP (successful login)", value=meta.get("source_ip", ""),
                    placeholder="10.0.0.5", key="meta_source_ip",
                )

            with st.expander("Behavioral Baseline", expanded=False):
                la = st.date_input(
                    "Last Active Date (leave blank if active recently)",
                    value=date.fromisoformat(meta["last_active_date"])
                        if meta.get("last_active_date") else None,
                    key="meta_last_active",
                )
                meta["last_active_date"] = str(la) if la else None
                rd = st.date_input(
                    "Resignation Date (leave blank if not applicable)",
                    value=date.fromisoformat(meta["resignation_date"])
                        if meta.get("resignation_date") else None,
                    key="meta_resignation",
                )
                meta["resignation_date"] = str(rd) if rd else None
                meta["normal_device_count"] = st.number_input(
                    "User's typical number of devices per session",
                    min_value=1, max_value=20,
                    value=int(meta.get("normal_device_count", 2)),
                    key="meta_normal_devices",
                )
            ...
            # ── Column header row ──────────────────────────────────────────────────
            st.markdown(
                '<div style="display:grid;grid-template-columns:2fr 1fr 1fr 3fr 2fr 2fr 1fr;'
                ...
                '<span style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;'
                'color:#7986a8;letter-spacing:.12em">LOCATION</span>'
                '<span style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;'
                'color:#7986a8;letter-spacing:.12em">RECORDS</span>'
                '<span style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;'
            ...
            for i, ev in enumerate(st.session_state.events):
                c1, c2, c3, c4, c5, c6, c7 = st.columns([2, 1, 1, 3, 2, 2, 1])
                with c1:
                    d = st.date_input(
                ...
                with c4:
                    dev_options = ["", " Custom "] + device_ids
                    stored_dev = ev["device"]
                    is_custom = stored_dev and stored_dev not in device_ids
                    display_idx = dev_options.index(" Custom ") if is_custom else (dev_options.index(stored_dev) if stored_dev in dev_options else 0)

                    dev_select = st.selectbox(
                        "Dev", dev_options, index=display_idx,
                        key=f"dev_{i}", label_visibility="collapsed",
                    )

                    if dev_select == " Custom ":
                        dev = st.text_input(
                            "Custom Device ID", value=stored_dev if is_custom else "",
                            key=f"dev_custom_{i}", label_visibility="collapsed",
                    ...
                    elif dev and dev not in device_ids:
                        st.markdown('<span style="font-family:\'Share Tech Mono\',monospace;font-size:.68rem;color:#7986a8">⚠ unknown device</span>', unsafe_allow_html=True)
                with c5:
                    loc = st.selectbox(
                        "Loc", INDIAN_CITIES,
                        index=INDIAN_CITIES.index(ev.get("location", "Mumbai"))
                            if ev.get("location", "Mumbai") in INDIAN_CITIES else 0,
                        key=f"loc_{i}", label_visibility="collapsed",
                    )
                    st.session_state.events[i]["location"] = loc
                with c6:
                    rec = st.number_input(
                        "Rec", min_value=0,
                        value=int(ev.get("records_accessed", 0)),
                        key=f"rec_{i}", label_visibility="collapsed",
                    )
                    st.session_state.events[i]["records_accessed"] = rec
                    records_total += rec
                with c7:
            ...
            st.markdown(
                ...
                f'color:#7986a8;margin:8px 0">Records Accessed (running total): '
                f'<span style="color:#4fc3f7">{records_total:,}</span></div>',
                unsafe_allow_html=True,
            )
            st.markdown(
                '<div style="font-family:\'Share Tech Mono\',monospace;font-size:.68rem;'
                'color:#3a5070;line-height:1.6;margin-bottom:8px">'
                'Tip: enter 3+ events with identical hour:minute to trigger Simultaneous Session detection<br>'
                'Tip: enter a VAULT- or DB- device followed by a WS- device to trigger Reverse Path detection'
                '</div>',
                unsafe_allow_html=True,
            )
        ...
        fingerprint = (
            ...
            + "|".join(
                ...
                f"-{e.get('location','')}-{e.get('records_accessed',0)}"
                for e in valid_events
            )
            + f"|meta={json.dumps(session_meta, sort_keys=True, default=str)}"
        ...
        if valid_events and engine_obj:
            if fingerprint_changed:
                ...
                try:
                    ev_list = [
                        build_event_dict(e, session_meta, is_first=(i == 0))
                        for i, e in enumerate(valid_events)
                    ]
                    opts = build_session_options(session_meta)
                    try:
                        output = engine_obj.analyzeSession(sel_user, ev_list, opts)
                    except TypeError:
                        output = engine_obj.analyzeSession(sel_user, ev_list)
                except Exception as ex:
                    output = f"[ERROR] {ex}"
                ...
                # Pull score_items from parsed output
                score_lines_parsed = re.findall(r'^\s+(.+?)\s+\+(\d+)\s*$', output, re.MULTILINE)
                st.session_state.last_score_items = [
                    {"reason": r.strip(), "points": int(s)}
                    for r, s in score_lines_parsed
                    if r.strip() not in ("TOTAL RISK SCORE", "Flag / Reason")
                ]
                ...
                if hist and hist[-1]["user"] == sel_user:
                    hist[-1] = {"user": sel_user, "score": score,
                                ...
                                "alert_level": al, "locked": score >= 75}
                else:
        ...
        if st.session_state.last_output:
            ...
            # ── Extended Threat Indicators (Cases 1–8) ────────────────────────────
            score_items = st.session_state.get("last_score_items", [])
            if not score_items and score_lines:
                score_items = [{"reason": r, "points": p} for r, p in score_lines]
            matched_threats = match_extended_threats(score_items)
            with st.expander("Extended Threat Indicators", expanded=True):
                st.markdown(
                    render_extended_threat_badges(matched_threats),
                    unsafe_allow_html=True,
                )
                ext_rows = []
                for name, triggered, pts, reason in matched_threats:
                    default_pts = next(p for n, _, p in EXTENDED_THREAT_CASES if n == name)
                    ext_rows.append({
                        "Case": name,
                        "Points": pts if triggered else 0,
                        "Max": default_pts,
                        "Status": "TRIGGERED" if triggered else "PASSED",
                        "Condition": reason if triggered else "No match",
                    })
                st.markdown(
                    render_events_table(
                        ext_rows,
                        ["Case", "Points", "Max", "Status", "Condition"],
                    ),
                    unsafe_allow_html=True,
                )
        ...
        if users:
            ...
            for u in users:
                ...
                rows_html += (
                    ...
                    f'<td style="padding:9px 14px;border-bottom:1px solid #2a3352;'
    f'font-family:\'Share Tech Mono\',monospace;font-size:.75rem;color:#4fc3f7">'
    + (
        "".join(
            f'<span style="display:inline-block;background:#0d1e38;'
            f'border:1px solid #2a3352;border-radius:3px;'
            f'padding:1px 7px;margin:2px 3px 2px 0;'
            f'font-family:\'Share Tech Mono\',monospace;font-size:.68rem;'
            f'color:#4fc3f7;letter-spacing:.05em">{dev}</span>'
            for dev in u.get("permitted_devices", [])
        ) if u.get("permitted_devices") else
        f'<span style="color:#3a5070;font-size:.72rem">— none —</span>'
    )
    + f'</td>'
                    f'</tr>'
                )
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    # PAGE: LIVE CLIENT EVENTS
    # ─────────────────────────────────────────────────────────────────────────────

    def _fetch_live_events():
        """Query the Flask event server for stored client events."""
        resp = requests.get(f"{EVENT_SERVER_URL}/events", timeout=5)
        resp.raise_for_status()
        return resp.json().get("events", [])

    def _fetch_live_results():
        try:
            return api_get("/live_results")
        except Exception:
            return {}

    def page_live_client_events():
        st.markdown(
            '<h2 style="font-family:\'Share Tech Mono\',monospace;color:#00e5ff;'
            'letter-spacing:.15em;font-size:1.25rem">📡 LIVE CLIENT EVENTS</h2>',
            unsafe_allow_html=True,
        )
        st.markdown(
            '<p style="color:#7986a8;font-size:.85rem;margin-bottom:18px">'
            'Real-time security events received from client machines via the Flask API '
            f'(<code style="color:#4fc3f7">{EVENT_SERVER_URL}</code>). '
            'Each incoming event triggers full C++ engine analysis automatically.</p>',
            unsafe_allow_html=True,
        )

        LIVE_USERS = ["PRIYA", "SNEHA", "RAJAN", "VIKRAM"]
        ...
        if "live_refresh_interval" not in st.session_state:
            st.session_state.live_refresh_interval = 10

        c_ref, c_pause, c_int = st.columns([1, 1, 2])
        with c_ref:
            manual_refresh = st.button("🔄  Refresh Now", use_container_width=True)
        with c_pause:
            st.session_state.live_auto_refresh = st.toggle(
                "Auto-refresh", value=st.session_state.live_auto_refresh,
            )
        with c_int:
        ...
        try:
            live_results = _fetch_live_results()
            events = _fetch_live_events()
            st.session_state.live_events_cache = events
            st.session_state.live_results_cache = live_results
            st.session_state.live_events_fetched_at = datetime.now()
            st.session_state.live_events_server_ok = True
            server_online = True
        except requests.exceptions.ConnectionError:
            server_error = (
                f"Cannot reach event server at {EVENT_SERVER_URL}. "
                "The Flask API starts automatically with `streamlit run app.py`."
            )
            events = st.session_state.get("live_events_cache", [])
            live_results = st.session_state.get("live_results_cache", {})
        except Exception as e:
            server_error = str(e)
            events = st.session_state.get("live_events_cache", [])
            live_results = st.session_state.get("live_results_cache", {})
        ...
        if server_error:
            st.markdown(
                f'<div style="color:#ffa726;font-family:\'Share Tech Mono\',monospace;'
                f'font-size:.75rem;padding:8px 0">◌ SERVER OFFLINE — {server_error}</div>',
                unsafe_allow_html=True,
            )
        else:
            ts = datetime.now().strftime("%H:%M:%S")
            st.markdown(
                f'<div style="color:#39ff14;font-family:\'Share Tech Mono\',monospace;'
                f'font-size:.75rem;padding:8px 0">● SERVER ONLINE — refreshed {ts}</div>',
                unsafe_allow_html=True,
            )

        st.markdown(sec_header("LIVE SCORES BY USER"), unsafe_allow_html=True)
        user_cards = []
        for u in LIVE_USERS:
            res = live_results.get(u, {})
            score = res.get("score")
            ...
            if score is None:
                ...
                display_val = f"{score} ({verdict})"
                color = live_verdict_color(verdict)
            user_cards.append(
                f'<div style="flex:1;background:#181c27;border:1px solid #2a3352;border-radius:8px;'
                f'padding:14px 16px;min-width:140px">'
                f'<div style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;'
                f'color:#7986a8;letter-spacing:.1em">{u}</div>'
                f'<div style="font-family:\'Share Tech Mono\',monospace;font-size:1.2rem;'
                f'color:{color};margin-top:6px">{display_val}</div></div>'
            )
        st.markdown(
            f'<div style="display:flex;gap:12px;flex-wrap:wrap;margin:12px 0">'
            f'{"".join(user_cards)}</div>',
            unsafe_allow_html=True,
        )
        ...
        for u in LIVE_USERS:
            ...
            if not res:
                continue
            with st.expander(f"📋 {u} — Live Analysis Detail (score {res.get('score', 0)})", expanded=False):
                st.markdown(
                    f"**Events in session:** {res.get('event_count', 0)} &nbsp;|&nbsp; "
                    f"**Last updated:** {res.get('last_updated', '—')} &nbsp;|&nbsp; "
                    f"**Email:** {'✓ Email Sent' if res.get('email_sent') else '— No Alert'}"
                )
                breakdown = res.get("score_breakdown", [])
                if breakdown:
                    st.markdown("**Score Breakdown**")
                    bd_rows = [{"Reason": b["reason"], "Points": b["points"]} for b in breakdown]
                    st.markdown(render_events_table(bd_rows, ["Reason", "Points"]), unsafe_allow_html=True)
                path = res.get("visited_path", [])
                if path:
                    st.markdown("**Visited Path**")
                    st.markdown(path_html(path), unsafe_allow_html=True)
                matched = match_extended_threats(breakdown)
                st.markdown(render_extended_threat_badges(matched), unsafe_allow_html=True)
                if st.button(f"Reset {u}", key=f"reset_live_{u}"):
                    try:
                        api_post(f"/reset_live/{u}")
                        st.success(f"Reset {u} live session")
                        st.rerun()
                    except Exception as ex:
                        st.error(str(ex))

        render_locked_accounts_panel(key_prefix="live_")

        st.markdown(sec_header("LIVE EVENT TICKER (last 20)"), unsafe_allow_html=True)
        if events:
            ticker_lines = []
            for ev in events[:20]:
                ts = ev.get("received_at", "—")
                user = ev.get("username", ev.get("username", "—"))
                dev = ev.get("device", "—")
                loc = ev.get("location", "—")
                rec = ev.get("records_accessed", 0)
                ip = ev.get("source_ip", "—")
                fail = ev.get("failed_attempts", 0)
                ticker_lines.append(
                    f'<div style="font-family:\'Share Tech Mono\',monospace;font-size:.72rem;'
                    f'padding:4px 0;border-bottom:1px solid #1e2335;color:#cdd6f4">'
                    f'{ts} | {user:<7} | {dev:<14} | {loc:<8} | records: {rec:<5} | '
                    f'ip: {ip} | failed: {fail}</div>'
                )
            st.markdown("".join(ticker_lines), unsafe_allow_html=True)
        else:
            st.info("No client events received yet. POST events to /events on the Flask API.")

        connected_machines = {ev.get("machine_name") for ev in events if ev.get("machine_name")}
        if connected_machines:
            st.markdown(sec_header("Connected Machines"), unsafe_allow_html=True)
            machine_list = ", ".join(sorted(connected_machines))
            st.markdown(
                f'<div style="font-family:\'Share Tech Mono\',monospace;font-size:.8rem;'
                f'color:#cdd6f4;padding:10px 14px;background:#181c27;'
                f'border:1px solid #2a3352;border-radius:6px">{machine_list}</div>',
                unsafe_allow_html=True,
            )

        if st.session_state.live_auto_refresh and not manual_refresh:
            import time
            time.sleep(st.session_state.live_refresh_interval)
            st.rerun()
    ...
    if "Configuration" in page:
        ...
        page_dashboard()
    elif "Live Client Events" in page:
        page_live_client_events()