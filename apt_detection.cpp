...
// ── Geographic helpers ───────────────────────────────────────────────────────
struct CityCoord { double lat, lon; };

const unordered_map<string, CityCoord> CITY_COORDS = {
...
double cityDistanceKm(const string& city1, const string& city2) {
    string c1 = toUpper(city1), c2 = toUpper(city2);
    auto it1 = CITY_COORDS.find(c1), it2 = CITY_COORDS.find(c2);
    if (it1 == CITY_COORDS.end() || it2 == CITY_COORDS.end()) return -1;
    return haversineKm(it1->second.lat, it1->second.lon,
                       it2->second.lat, it2->second.lon);
}

int daysBetweenDates(const string& d1, const string& d2) {
    struct tm t1, t2;
    if (!parseDate(d1, t1) || !parseDate(d2, t2)) return -1;
...
int getDeviceTier(const string& device, const NetworkGraph& graph,
                  const ZoneStore& zones) {
    string u = toUpper(device);
    if (u.find("VAULT-") != string::npos || u.find("DB-") != string::npos) return 4;
    if (u.find("AUDIT-") != string::npos) return 3;
    if (u.find("SRV-") != string::npos || u.find("INTRA-") != string::npos) return 2;
    if (u.find("WS-") != string::npos) return 1;
    auto it = graph.nodes.find(u);
    if (it != graph.nodes.end()) {
        string zone = toUpper(it->second.zone);
        if (zone == "WORKSTATION" || zone == "BRANCH") return 1;
        if (zone == "INTERNAL" || zone == "CORE") return 2;
    }
    return 2;
}
...
public:
    ...
    unordered_map<string, string>         userLastActiveDate;
    unordered_map<string, string>         userResignationDate;
    unordered_map<string, int>            userAvgDailyRecords;
    unordered_map<string, int>            userNormalDeviceCount;
    unordered_map<string, string>         userLastLoginLocation;
    ...
    void addUser(const string& user, const string& role,
        ...
        if (!userLastActiveDate.count(uuser))    userLastActiveDate[uuser]    = "";
        if (!userResignationDate.count(uuser))   userResignationDate[uuser]   = "";
        if (!userAvgDailyRecords.count(uuser))   userAvgDailyRecords[uuser]   = 50;
        if (!userNormalDeviceCount.count(uuser)) userNormalDeviceCount[uuser] = 2;
        if (!userLastLoginLocation.count(uuser)) userLastLoginLocation[uuser] = "";
    }
    void setUserMeta(const string& user, const string& lastActive,
                     const string& resigned, int avgRecords, int normalDevices) {
        string u = toUpper(user);
        if (!lastActive.empty() && lastActive != "none" && lastActive != "-")
            userLastActiveDate[u] = lastActive;
        if (!resigned.empty() && resigned != "none" && resigned != "-")
            userResignationDate[u] = resigned;
        if (avgRecords > 0) userAvgDailyRecords[u] = avgRecords;
        if (normalDevices > 0) userNormalDeviceCount[u] = normalDevices;
    }
    string getLastActiveDate(const string& user) const {
        auto it = userLastActiveDate.find(toUpper(user));
        return it == userLastActiveDate.end() ? "" : it->second;
    }
    string getResignationDate(const string& user) const {
        auto it = userResignationDate.find(toUpper(user));
        return it == userResignationDate.end() ? "" : it->second;
    }
    int getAvgDailyRecords(const string& user) const {
        auto it = userAvgDailyRecords.find(toUpper(user));
        return it == userAvgDailyRecords.end() ? 50 : it->second;
    }
    int getNormalDeviceCount(const string& user) const {
        auto it = userNormalDeviceCount.find(toUpper(user));
        return it == userNormalDeviceCount.end() ? 2 : it->second;
    }
    string getLastLoginLocation(const string& user) const {
        auto it = userLastLoginLocation.find(toUpper(user));
        return it == userLastLoginLocation.end() ? "" : it->second;
    }
    void setLastLoginLocation(const string& user, const string& loc) {
        if (!loc.empty()) userLastLoginLocation[toUpper(user)] = loc;
    }
    ...
    bool removeUser(const string& user) {
        ...
        userLastActiveDate.erase(u);
        userResignationDate.erase(u);
        userAvgDailyRecords.erase(u);
        userNormalDeviceCount.erase(u);
        userLastLoginLocation.erase(u);
        return true;
    }
};
...
// ─────────────────────────────────────────────────────────────
struct SessionEvent {
    string device, date, location, sourceIP, lastFailedIP;
    int hour, minute, failedAttempts, recordsAccessed;
    SessionEvent()
        : hour(0), minute(0), failedAttempts(0), recordsAccessed(0),
          location("Mumbai") {}
};

long long eventMinutesSinceEpoch(const SessionEvent& ev) {
    struct tm t;
    if (!parseDate(ev.date, t)) return 0;
    t.tm_hour = ev.hour;
    t.tm_min  = ev.minute;
    t.tm_sec  = 0;
...
struct SessionOptions {
    string lastActiveDateOverride;
    string resignationDateOverride;
    int normalDeviceCountOverride;
    int failedAttempts;
    string lastFailedIP;
    string sourceIP;
    SessionOptions()
        : normalDeviceCountOverride(-1), failedAttempts(0) {}
};
...
public:
    ...
    void analyseSession(const string& user, const vector<SessionEvent>& events,
        ...
        // ── CASE 1: Geographic Jump ───────────────────────────────────────
        for (size_t gi = 1; gi < events.size(); ++gi) {
            const SessionEvent& prev = events[gi - 1];
            const SessionEvent& curr = events[gi];
            if (prev.location.empty() || curr.location.empty()) continue;
            double dist = cityDistanceKm(prev.location, curr.location);
            if (dist < 0) continue;
            long long tPrev = eventMinutesSinceEpoch(prev);
            long long tCurr = eventMinutesSinceEpoch(curr);
            long long mins  = (tCurr - tPrev) / 60;
            if (mins < 0) mins = -mins;
            if (dist >= 1000.0 && mins < 240) {
                score += 50;
                ostringstream geoReason;
                geoReason << "Geographic jump: " << prev.location << " -> " << curr.location
                          << " (" << (int)dist << "km in " << mins << "min)";
                scoreItems.push_back({geoReason.str(), 50});
                break;
            }
        }

        // ── CASE 2: Simultaneous Sessions ─────────────────────────────────
        {
            unordered_map<string, vector<string>> minuteDevices;
            for (size_t si = 0; si < events.size(); ++si) {
                const SessionEvent& ev = events[si];
                ostringstream key;
                key << ev.date << "|" << setfill('0') << setw(2) << ev.hour
                    << ":" << setw(2) << ev.minute;
                string udev = toUpper(ev.device);
                if (!udev.empty()) minuteDevices[key.str()].push_back(udev);
            }
            for (auto it = minuteDevices.begin(); it != minuteDevices.end(); ++it) {
                set<string> uniqueDevs(it->second.begin(), it->second.end());
                if ((int)uniqueDevs.size() >= 3) {
                    score += 45;
                    ostringstream simReason;
                    simReason << "Simultaneous sessions on " << uniqueDevs.size()
                              << " devices at " << it->first << ": ";
                    bool first = true;
                    for (const string& d : uniqueDevs) {
                        if (!first) simReason << ", ";
                        simReason << d; first = false;
                    }
                    scoreItems.push_back({simReason.str(), 45});
                    break;
                }
            }
        }

        // ── CASE 3: Dormant Account Activation ────────────────────────────
        {
            string lastActive = opts.lastActiveDateOverride;
            if (lastActive.empty()) lastActive = perms.getLastActiveDate(uuser);
            if (!lastActive.empty() && !events.empty()) {
                int dormDays = daysBetweenDates(lastActive, events[0].date);
                if (dormDays >= 90) {
                    score += 35;
                    scoreItems.push_back({
                        "Dormant account reactivated after " + to_string(dormDays)
                        + " days (last active: " + lastActive + ")", 35});
                    if (!events[0].device.empty()) {
                        string firstDev = toUpper(events[0].device);
                        SecurityLevel firstSec = SecurityLevel::UNKNOWN_SL;
                        if (graph.nodes.count(firstDev))
                            firstSec = zones.getLevel(graph.nodes.at(firstDev).zone);
                        if (firstSec == SecurityLevel::HIGH) {
                            score += 15;
                            scoreItems.push_back({
                                "Dormant account immediate HIGH-sec access: " + firstDev, 15});
                        }
                    }
                }
            }
        ...
        // ── CASE 4: Post-Resignation Access ───────────────────────────────
        {
            string resignDate = opts.resignationDateOverride;
            if (resignDate.empty()) resignDate = perms.getResignationDate(uuser);
            if (!resignDate.empty()) {
                for (size_t ri = 0; ri < events.size(); ++ri) {
                    if (daysBetweenDates(resignDate, events[ri].date) >= 0) {
                        score += 60;
                        scoreItems.push_back({
                            "Post-resignation access: user resigned on " + resignDate
                            + ", activity on " + events[ri].date, 60});
                        break;
                    }
                }
            }
        ...
        // ── CASE 5: Failed-Then-Success Pattern ───────────────────────────
        {
            int failCount = opts.failedAttempts;
            string failIP = opts.lastFailedIP;
            string succIP = opts.sourceIP;
            if (failCount == 0 && !events.empty()) {
                failCount = events[0].failedAttempts;
                if (failIP.empty()) failIP = events[0].lastFailedIP;
                if (succIP.empty()) succIP = events[0].sourceIP;
            }
            if (failCount >= 10 && !failIP.empty() && !succIP.empty()
                && toUpper(failIP) != toUpper(succIP)) {
                score += 55;
                scoreItems.push_back({
                    "Brute force pattern: " + to_string(failCount)
                    + " failed attempts from " + failIP + ", success from " + succIP, 55});
            }
        }

        // ── CASE 6: Data Volume Spike ─────────────────────────────────────
        {
            int totalRecords = 0;
            for (size_t di = 0; di < events.size(); ++di)
                totalRecords += events[di].recordsAccessed;
            int baseline = perms.getAvgDailyRecords(uuser);
            if (baseline > 0 && totalRecords > baseline * 100) {
                score += 50;
                int mult = totalRecords / baseline;
                scoreItems.push_back({
                    "Data volume spike: " + to_string(totalRecords)
                    + " records accessed (baseline: " + to_string(baseline)
                    + "/day, " + to_string(mult) + "x normal)", 50});
            }
        }

        // ── CASE 7: Behavioral Drift ────────────────────────────────────
        {
            set<string> distinctDevs;
            for (size_t bi = 0; bi < visitedPath.size(); ++bi)
                distinctDevs.insert(visitedPath[bi]);
            int baseline = opts.normalDeviceCountOverride >= 0
                         ? opts.normalDeviceCountOverride
                         : perms.getNormalDeviceCount(uuser);
            if ((int)distinctDevs.size() > baseline * 2) {
                score += 40;
                scoreItems.push_back({
                    "Behavioral drift: " + to_string(distinctDevs.size())
                    + " distinct devices accessed (normal: " + to_string(baseline) + ")", 40});
            }
        }

        // ── CASE 8: Reverse Path Access ───────────────────────────────────
        {
            int reversalCount = 0;
            for (size_t pi = 1; pi < visitedPath.size() && reversalCount < 2; ++pi) {
                int prevTier = getDeviceTier(visitedPath[pi - 1], graph, zones);
                int currTier = getDeviceTier(visitedPath[pi], graph, zones);
                if (prevTier - currTier >= 2) {
                    score += 45;
                    reversalCount++;
                    scoreItems.push_back({
                        "Reverse path access: " + visitedPath[pi - 1] + " -> "
                        + visitedPath[pi]
                        + " (accessed vault/DB before workstation)", 45});
                }
            }
        }

        // Update last login location from final event
        if (!events.empty() && !events.back().location.empty())
            perms.setLastLoginLocation(uuser, events.back().location);

        string verdict = score>=30 ? "*** CRITICAL THREAT ***" : score>=10 ? "SUSPICIOUS" : "CLEAR";

        cout << "\n=== SESSION ANALYSIS: " << sid << " ===\n";
        cout << "User: " << uuser << "  Role: " << role << "\n";
        cout << "Flag / Reason                                    Points\n";
        cout << "--------------------------------------------------------\n";
        for (size_t si = 0; si < scoreItems.size(); ++si)
            cout << "  " << scoreItems[si].reason << "  +" << scoreItems[si].points << "\n";
        cout << "--------------------------------------------------------\n";
        cout << "TOTAL RISK SCORE          " << score << "\n";
        cout << "VERDICT: " << verdict << "\n";
        if (visitedPath.size() >= 2) {
            vector<string> bfsPath = bfs.findPath(graph, visitedPath.front(), visitedPath.back());
            if (!bfsPath.empty()) {
                cout << "BFS shortest path : ";
                for (size_t bi = 0; bi < bfsPath.size(); ++bi) {
                    if (bi) cout << " --> ";
                    cout << bfsPath[bi];
                }
                cout << "\n";
        ...
        cout << "Visited path: ";
        for (size_t vi = 0; vi < visitedPath.size(); ++vi) {
            if (vi) cout << " -> ";
            cout << visitedPath[vi];
        }
        cout << "\n";
...
void loadConfig(const string& filename, NetworkGraph& graph,
    ...
    while (getline(f, line)) {
        ...
        if (cmd=="ZONE") {
            ...
            string lastActive, resigned;
            int avgRecords = 50, normalDevices = 2;
            while (ss >> p) {
                size_t colon = p.find(':');
                if (colon != string::npos) {
                    string key = toUpper(p.substr(0, colon));
                    string val = p.substr(colon + 1);
                    if (key == "LAST_ACTIVE") lastActive = val;
                    else if (key == "RESIGNED") resigned = val;
                    else if (key == "AVG_RECORDS") avgRecords = stoi(val);
                    else if (key == "NORMAL_DEVICES") normalDevices = stoi(val);
                } else {
                    permitted.push_back(p);
                }
            }
            perms.addUser(user,role,machine,permitted);
            perms.setUserMeta(user, lastActive, resigned, avgRecords, normalDevices);
            loaded++;
        } else if (cmd=="APT") {
...
void saveConfig(const string& filename, const NetworkGraph& graph,
    ...
    for (auto it=perms.userRoles.begin();it!=perms.userRoles.end();++it) {
        ...
        string la = perms.getLastActiveDate(u);
        string rd = perms.getResignationDate(u);
        if (!la.empty()) f << " LAST_ACTIVE:" << la;
        if (!rd.empty()) f << " RESIGNED:" << rd;
        f << " AVG_RECORDS:" << perms.getAvgDailyRecords(u);
        f << " NORMAL_DEVICES:" << perms.getNormalDeviceCount(u);
        f << "\n";
    }
