...
public:
    ...
    void analyseSession(const string& user, const vector<SessionEvent>& events) {
        ...
        if (score >= EMAIL_THRESHOLD) {
            string subject = "[SECUREBANK ALERT] Critical Threat - User: "+uuser+" | Score: "+to_string(score);
            string body = "SECUREBANK APT DETECTION ALERT\nSession: "+sid+"  User: "+uuser+"  Role: "+role+"\n"
                          "Score: "+to_string(score)+"  Verdict: "+verdict+"\n\nFLAGS:\n";
            for (size_t i = 0; i < scoreItems.size(); ++i)
                body += "  [+"+to_string(scoreItems[i].points)+"] "+scoreItems[i].reason+"\n";
            body += "\nPath: ";
            for (int i = 0; i < (int)visitedPath.size(); ++i) { if (i) body+=" --> "; body+=visitedPath[i]; }
            body += "\n\nTimestamp: "+currentTimestamp();
            cout << "\n[EMAIL] Score " << score << " >= threshold " << EMAIL_THRESHOLD << " -- sending alert...\n";
            sendEmailAlert(ALERT_EMAIL, subject, body);
        }
    }
};
